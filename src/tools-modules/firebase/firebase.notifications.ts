...
import { Message, MulticastMessage, Notification } from 'firebase-admin/messaging';
...
export class FirebaseNotifications {
  ...
  async sendToDevices(deviceTokens: string[], payload: { notification?: Notification, data?: Record<string, string> }): Promise<Set<string>> {
    ...
    for (let i = 0; i < deviceTokens.length; i += BATCH_SIZE) {
      ...
      const response = await this.firebaseApp.messaging().sendEachForMulticast(pushNotification);
  ...
  async sendToTopic(topic: string, payload: { notification?: Notification, data?: Record<string, string> }): Promise<void> {
